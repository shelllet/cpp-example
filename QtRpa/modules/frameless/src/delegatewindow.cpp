...
#include "frameless/delegatewindow.h"
#include "common/private/object_p.h"
#include "frameless/titlebarwidget.h"
	...
	: Object(*new DelegateWindowPrivate, nullptr)
{
	...
	if (!m_pDirect2dFactory) {
		...
		if (FAILED(hResult)) {
			LOG_WARNING("frameless") << "Failed to create d2d factory: " << std::system_category().message(hResult) << std::endl;
		}
	}
}
...
int shelllet::frameless::DelegateWindow::getSystemTitleBarHeight(HWND hWnd)
{
	...
	if (IsZoomed(hWnd)) {
		return qRound(__GetSystemMetricsForDpi(SM_CYCAPTION, dpi) * 0.75);
	}

	return __GetSystemMetricsForDpi(SM_CYCAPTION, dpi);
}
