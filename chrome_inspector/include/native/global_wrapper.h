...
namespace shelllet {
	namespace global {
		...
		public:
			...
			MyGlobalWrapper(v8::Isolate* isolate, const v8::Local<v8::FunctionTemplate>& tpl) : ObjectWrapper(isolate, tpl)
			{
			}
			MyGlobalWrapper(v8::Isolate* isolate, const v8::Local<v8::ObjectTemplate>& proto) : ObjectWrapper(isolate, proto)
			{
		...
		protected:
			...
			static void SetTimeout(const v8::FunctionCallbackInfo<v8::Value>& args) {
				V8_CREATE_LOCAL_CONTEXT_WITHOUT_LOCKER_BY_CALLBACK_INFO(args)
				{
					...
					TimerPrivate* p = new TimerPrivate(isolate, args[0].As<v8::Function>());
					p->setTimeout([isolate](TimerPrivate* p) {
			...
			static void ClearTimeout(const v8::FunctionCallbackInfo<v8::Value>& args)
			{
				V8_CREATE_LOCAL_CONTEXT_WITHOUT_LOCKER_BY_CALLBACK_INFO(args)
				{
					...
					if (auto* timer = static_cast<TimerPrivate*>(args[0].As<v8::External>()->Value()); timer)
					{
						timer->clearTimeout();
					}
				}
			...
			static void SetInterval(const v8::FunctionCallbackInfo<v8::Value>& args)
			{
				V8_CREATE_LOCAL_CONTEXT_WITHOUT_LOCKER_BY_CALLBACK_INFO(args)
				{
					...
					TimerPrivate* p = new TimerPrivate(isolate, args[0].As<v8::Function>());
					p->setInterval([isolate](TimerPrivate* p) {
						V8_CREATE_LOCAL_LOCKER_CONTEXT_BY_ISOLATE(isolate)
						{
							v8::Local<v8::Function> fn = p->fn.Get(isolate);
							if (!fn.IsEmpty() && fn->IsFunction())
