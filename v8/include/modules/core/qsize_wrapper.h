...
namespace shelllet {
	namespace core {
		...
		class QSizeWrapper : public ObjectWrapper<T, U, C>
		{
		public:
			template<bool M>
			QSizeWrapper(QSizePrivate<T, M>& d, const v8::FunctionCallbackInfo<v8::Value>& args)
				: ObjectWrapper(d, args) {
			}
			QSizeWrapper(v8::Isolate* isolate, const v8::Local<v8::FunctionTemplate>& tpl) : ObjectWrapper(isolate, tpl) {}
			QSizeWrapper(v8::Isolate* isolate, const v8::Local<v8::ObjectTemplate>& proto) : ObjectWrapper(isolate, proto)
			{
		...
		protected:
			static void GetWidth(v8::Local<v8::String> property, const v8::PropertyCallbackInfo<v8::Value>& info)
			{
				...
				{
					auto* p = UnWrap<QSizeWrapper<T, U, C>>(info.This());
					if (!p)
						return;
			...
			static void BoundedTo(const v8::FunctionCallbackInfo<v8::Value>& args)
			{
				...
				{
					auto* p = UnWrap<QSizeWrapper<T, U, C>>(args.This());
					if (!p) {
						return;
					}
			...
			static void Transposed(const v8::FunctionCallbackInfo<v8::Value>& args)
			{
				V8_CREATE_LOCAL_CONTEXT_WITHOUT_LOCKER_BY_CALLBACK_INFO(args)
				{
					auto* self = UnWrap<QSizeWrapper<T, U, C>>(args.This());
					if (!self) {
						return;
					}
